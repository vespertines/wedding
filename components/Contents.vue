<template>
  <div class="container">
    <div class="row">
      <div class="col-6 contents timetable">
        <h3>Timetable</h3>
        <table>
          <tr>
            <td>2:00 pm</td>
            <td>Arrival</td>
          </tr>
          <tr>
            <td>2:30 pm</td>
            <td>Wedding Ceremony</td>
          </tr>
          <tr>
            <td>3:10 pm</td>
            <td>Photos</td>
          </tr>
          <tr>
            <td>3:30 pm</td>
            <td>Drinks & Buffet Reception</td>
          </tr>
          <tr>
            <td>5:30 pm</td>
            <td>The Chartered Buses Leave</td>
          </tr>
          <tr>
            <td>6:10 pm</td>
            <td>Break Time</td>
          </tr>
          <tr>
            <td>7:00 pm</td>
            <td>After Party</td>
          </tr>
          <tr>
            <td>10:00 pm</td>
            <td>Farewell</td>
          </tr>
        </table>
      </div>

      <div class="col-6 contents">
        <h3>Travel</h3>
        <p>
          The full address is 745 Seonggok-dong, Andong-si, Gyeongsangbuk-do or
          190 Minsokchon-gil, Andong-si, Gyeongsangbuk-do.
        </p>
        <p>
          There will be chartered buses from Seoul and Busan in the morning
          (around 9am). We will share the exact times and location a month before
          the wedding. Please keep a watch out for further instructions.
        </p>
        <p>
          Also if you would like to take public transportation and take off from
          Andong train / bus station, there is a city bus 3 or take a taxi and the
          ticket should cost less than 15,000 won.
        </p>
        <p>
          We recommend you to use Naver map (mobile) rather than Google map. Read
          an article.
        </p>
      </div>

      <div class="col-6 contents">
        <h3>Accommodation</h3>
        <p>
          For guests travelling in, if you would like to enjoy traditional Korean
          accommodations, we recommend you to stay at Gurume Resort with us. The
          resort has limited rooms so R.S.V.P. Let us know via email if you
          require additional details.
        </p>
        <p>
          Otherwise Andong Grand Hotel or Richell Hotel are alternative
          accommodations.
        </p>
        <p>Feel free to send an email if you need any help booking accommodations.</p>
      </div>

      <div class="col-6 contents">
        <h3>Jeju Island</h3>
        <p>
          We’re planning to go to Jeju Island a few days to chill and relax after
          the wedding (17 - 21 May). there are a few flight departing from Gimhae
          airport. It would be great if you would join us there!
        </p>
        <p>
          It is a very beautiful island with a unique and special culture. We
          highly recommend a visit there while you’re in South Korea.
        </p>
        <p>
          If it becomes a group we suggest to stay here (7-10 Max / up to £370 per
          day) but if it becomes larger number we can find other places or happy
          to have an accommodation separately.
        </p>
        <p>
          Also it depends on number of people we can plan how to get the airport
          together.
        </p>
      </div>

      <div class="col-6 contents">
        <h3>Car Parking</h3>
        <p>
          Car parking is available at the venue and cars can be left at the venue
          overnight.
        </p>
      </div>
    </div>
  </div>
</template>

<style lang="scss" scoped>
$white: #ffffff;
$black: #000000;
$grey: #505050;
$grey-light: #7e7e7e;

.contents {
  padding-bottom: 90px;

  h3 {
    margin-bottom: 20px;
    color: $black;
    font-size: 15px;
    font-weight: normal;
    line-height: 1em;
  }

  p {
    color: $grey-light;
    font-size: 15px;
  }
}

.timetable {
  tr {
    td {
      font-size: 15px;
      line-height: 1em;

      &:nth-of-type(1) {
        width: 90px;
      }

      &:nth-of-type(2) {
        width: calc(100% - 90px);
        color: $grey-light;
      }
    }
  }
}
</style>
